...
def saveOptions():
    for i in range (0,26):
        variables.selectedColorOptions[1][i] = cb_color[i].get()
    for i in range (0,11):
        variables.selectedSizeOptions[1][i] = cb_size[i].get()
    mw.destroy()
...
def run():
    ...
    #Title
    Label(mw, text ="Round Neck Full Sleeves Tshirt Options", font = "none 12 bold").place(height = 25, width = 450)
    #Select Colour
